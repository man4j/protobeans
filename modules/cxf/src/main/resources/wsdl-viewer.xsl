...
<xsl:param name="ENABLE-SERVICE-PARAGRAPH" select="true()"/>
   ...
   <xsl:variable name="SOURCE-CODE-TEXT">Исходный код</xsl:variable>
...
-->
   ...
   <xsl:template match="ws:operation|ws2:operation" mode="service">
         ...
         <xsl:if test="$ENABLE-LINK">
		          <xsl:if test="$ENABLE-OPERATIONS-PARAGRAPH">
               <a class="local" href="{concat('#', $OPERATIONS-PREFIX, generate-id(.))}">Подробности</a>
            </xsl:if> 
            <xsl:call-template name="render.source-code-link"/>
         </xsl:if>
	     </li>
   ...
   <xsl:template match="xsd:attribute[ @*[local-name() = 'arrayType'] ]"
    ...
    <xsl:template name="xsd.element-type">
        ...
        <xsl:variable name="type-local-name">
            <xsl:call-template name="ns-type-local-name">
                <xsl:with-param name="name" select="$ref-or-type"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="type-name">
            <xsl:choose>
    ...
    <xsl:template name="ns-type-local-name">
        <xsl:param name="name"/>
        <xsl:if test="contains($name, ':')">
            <xsl:variable name="ns">
                <xsl:call-template name="get-ns-name">
                    <xsl:with-param name="ns-prefix" select="substring-before($name, ':')"/>
                </xsl:call-template>
            </xsl:variable>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:variable name="elements" select="$consolidated-xsd[@name = $local-name]"/>
                    <xsl:for-each select="$elements">
                        <xsl:choose>
                            <xsl:when test="local-name() = 'element'">
                                <xsl:variable name="curr-ns">
                                    <xsl:call-template name="get-ns-name">
                                        <xsl:with-param name="ns-prefix" select="substring-before(@type, ':')"/>
                                    </xsl:call-template>
                                </xsl:variable>
                                <xsl:if test="$curr-ns = $ns">
                                    <xsl:value-of select="substring-after(@type, ':')"/>
                                </xsl:if>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:if test="$ns = ../@targetNamespace">
                                    <xsl:value-of select="$local-name"/>
                                </xsl:if>
                            </xsl:otherwise>
                        </xsl:choose>
			         ...
			         <small style="color:blue">
				           ...
				           <xsl:choose>
					             <xsl:when test="$elem-type">
                                     ...
                                     <xsl:apply-templates select="$elem-type/xsd:annotation/xsd:documentation" mode="operations.message.part"/>
					             </xsl:when>
					             <xsl:otherwise>
