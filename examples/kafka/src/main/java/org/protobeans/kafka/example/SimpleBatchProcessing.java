...
@EnableKafkaMessaging(brokerList = "127.0.0.1:19092", concurrency = 1)
public class SimpleBatchProcessing {
    ...
    @KafkaListener(id = "group1", topics = "topic1", batch = "true")
    public void topicListener(ConsumerRecords<String, String> records) {
        System.out.println("Start batch processing--------------------------");
        for (var record : records) {
            System.out.println("Thread: " + Thread.currentThread().getName() 
                             + ", Partition: " + record.partition() 
                             + ", Value: " + record.value());
        }
        System.out.println("End batch processing--------------------------");
    ...
    public static void main(String[] args) {
        EntryPoint.run(SimpleBatchProcessing.class);
    }
}